...
_TARGET_CHARS_MIN = 950
_TARGET_CHARS_MAX = 1050
...
def generate_clone_script(
    ...
    *,
    ...
    char_count = len(text)
    # Allow up to 30% slack — the user can manually trim/extend in the UI.
    floor = int(_TARGET_CHARS_MIN * 0.7)
    ceiling = int(_TARGET_CHARS_MAX * 1.3)
    if char_count < floor or char_count > ceiling:
        raise ScriptGenError(
            f"sample length out of range: {char_count} chars "
            f"(expected {_TARGET_CHARS_MIN}-{_TARGET_CHARS_MAX})",
        )
    return text
